...
export const description = "A gas resistance line chart"

const chartConfig = {
  gas_resistance: {
    label: "გაზის წინაღობა",
    color: "hsl(45, 93%, 47%)", // Yellow-400 color to match theme
  },
} satisfies ChartConfig

export function ChartGasResistance() {
  ...
  return (
    <Card className="@container/card transition-all duration-300 hover:scale-[1.02] hover:shadow-lg hover:shadow-yellow-500/20">
      <CardHeader>
        <CardTitle>გაზის წინაღობის გრაფიკი</CardTitle>
        <CardDescription>
          <span className="hidden @[540px]/card:block">
            რეალურ დროში გაზის მონაცემები
          </span>
        ...
        {isLoading && filteredData.length === 0 ? (
          ...
          <ChartContainer config={chartConfig} className="h-[200px] w-full">
            ...
            <Line
              ...
              type="natural"
              stroke="var(--color-gas_resistance)"
              strokeWidth={2}
              dot={false}
            />
          </LineChart>
